...
const sendMessage = (message) => {
  ...
  eventSource.onmessage = (event) => {
    ...
    // 判断是否是结构化消息
    if (parsed === null) {
      // parseMessage 返回 null 表示忽略此消息（如只有 thought 的消息）
      return
    }

    if (typeof parsed === 'object' && parsed.messageType) {
      // 结构化消息，立即添加到消息列表
      addMessage(parsed, false, parsed.messageType, true)

      // 更新连接状态
      if (parsed.messageType === MessageType.ERROR) {
        connectionStatus.value = 'error'
      } else if (parsed.messageType === MessageType.SUMMARY) {
        connectionStatus.value = 'disconnected'
      } else {
